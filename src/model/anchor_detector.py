...
class AnchorDetector(nn.Module):
    ...
    def forward(
        ...
        runtime_score = torch.sigmoid(runtime_raw)

        prior_logits = self.prior_head(hidden).squeeze(-1)
        prior_score = torch.sigmoid(prior_logits)

        combined = self.prior_weight * prior_score + self.runtime_weight * runtime_score
        scores = torch.sigmoid(combined)
        semantic_weights = combined
