...
from src.fog.model_structured import StructuredMotifTransformer, build_layer_geometries


def test_build_layer_geometries_has_depth_stages() -> None:
    cfg = FOGConfig(
        vocab_size=64,
        d_model=96,
        n_layers=6,
    ...
    geometries = build_layer_geometries(cfg)
...
def test_structured_motif_forward_backward() -> None:
    cfg = FOGConfig(
        vocab_size=64,
        d_model=96,
        n_layers=6,
    ...
    model = StructuredMotifTransformer(cfg)
