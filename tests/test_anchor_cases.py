...
def test_anchor_probe_shows_basic_separation():
    ...
    assert stable_diag['mean_descendant_mass'] < complex_diag['mean_descendant_mass']
    assert stable_diag['state_counts']['confirmed'] >= complex_diag['state_counts']['confirmed']
    assert stable_diag['dead_end_count'] <= complex_diag['dead_end_count']
