"""
Experiment 2: Cross-Layer Angular Drift (CLAD)
Measures how representations drift through FFN/MFFN layers.
"""
import math
import random
import time
...
class BaselineT(nn.Module):
    ...
    def forward(self, ids, tgt=None, lm=None):
        ...
        for bl in self.blocks:
            ...
            # CLAD tracking for FFN
            x_in = x.clone()
            ffn_out = bl['ffn'](bl['ln2'](x))
            x = x + ffn_out
            
            # Cosine similarity between input to block and output of block
            sim = F.cosine_similarity(x_in, x, dim=-1).mean().item()
            clad_scores.append(1.0 - sim) # Drift is 1 - similarity
        ...
        return {"logits": lo, "loss": loss, "clad": clad_scores}
...
@torch.no_grad()
def eval_and_clad(model, loader, dev):
    model.eval()
    cor, tot = 0, 0
    avg_clad = None
    batches = 0
    for b in loader:
        ...
        if avg_clad is None:
            ...
            avg_clad = [a + b_val for a, b_val in zip(avg_clad, o["clad"])]
        batches += 1
        
    avg_clad = [x / batches for x in avg_clad]
    return cor / max(tot, 1), avg_clad

def run_experiment():
    dev = torch.device("cpu")
    print(f"Running CLAD Experiment on {dev}\n")
    ...
    # We use a solid parameter size to see the true geometric difference without starving
    d_model = 128
    
    # Uniform
    m_uniform = BaselineT(d_model=d_model, d_ff=d_model*4, n_heads=4, n_layers=4, vocab_size=vocab_size, max_seq_len=seq_len).to(dev)
    
    # FOG Motif
    m_fog = MotifT(
        d_model=d_model, d_compare=32, d_memory=96, d_expand=256, d_gate=16, 
    ...
    for name, model in [("Uniform", m_uniform), ("FOG", m_fog)]:
        ...
        for ep in range(1, epochs + 1):
            ...
            if ep % 5 == 0 or ep == 1:
                acc, clad = eval_and_clad(model, el, dev)
                print(f"  Ep {ep:2d} | Acc: {acc:.3f} | CLAD Layers: {[round(c, 4) for c in clad]}")
                clad_history.append({"epoch": ep, "clad": clad})
                
        results[name] = clad_history

    out_path = Path("fog_exp2_clad.json")
    out_path.write_text(json.dumps(results, indent=2))
    print(f"\nSaved CLAD results to {out_path}")
