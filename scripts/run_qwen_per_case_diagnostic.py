"""Minimal per-case tail_retention diagnostic — medium profile."""
...
def main():
    ...
    torch.manual_seed(7)
    print(f"[Diag] Loading {args.model_name} on {dev}")
    ov = QwenAnchorOverlay.from_pretrained(args.model_name, config=TOY_CONFIG)
    ov.to(dev); ov.eval()
    nl = int(ov.model_num_hidden_layers)
    all_cases = make_qwen_anchor_geometry_cases(anchor_span_profile="medium")
    ...
    result = {"spearman_rho": rho, "n_valid": len(trs), "n_total": len(cases), "n_source_cases": len(all_cases), "mean_tr": round(float(np.mean(trs)),4) if trs else None, "mean_cs": round(float(np.mean(css)),4) if css else None, "model": args.model_name, "profile": "medium", "max_new_tokens": args.max_new_tokens, "group_case_cap": args.group_case_cap, "cases": rows}
    Path(ROOT/"archive").mkdir(exist_ok=True)
    Path(ROOT/"archive/qwen35_4b_per_case_diagnostic.json").write_text(json.dumps(result, indent=2, ensure_ascii=False))
    print("\n===FINAL_RESULT===")
